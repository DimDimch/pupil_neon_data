...
# Игнорируем ошибки импорта для Pupil Labs
...
# ────────────────────────── Ввод данных (Метаданные) ─────────────────────
try:
    print("\n==========================================")
    print("      📝 РЕГИСТРАЦИЯ ИСПЫТУЕМОГО          ")
    print("==========================================")
    subject_id = input("1. ID испытуемого (напр. Ivan_01): ").strip()
    if not subject_id: subject_id = "Unknown"

    group_id = input("2. Группа (напр. Patient): ").strip()
    if not group_id: group_id = "None"

    age = input("3. Возраст: ").strip()
    gender = input("4. Пол (M/F): ").strip()
    session = input("5. Номер сессии: ").strip()

    view_input = input("6. Расстояние до экрана (в см): ")
    VIEW_DIST_CM = float(view_input) if view_input else 62.5
    print("==========================================\n")
except ValueError:
    print("❌ Ошибка ввода. Используем стандартное расстояние 62.5 см.")
    VIEW_DIST_CM = 62.5
...
# ─────── Маркеры углов и периметра (Surface) ───────
...
# 🔥 МАГИЯ: АВТОМАТИЧЕСКАЯ ОТРИСОВКА РАМКИ 🔥
# Переопределяем flip, чтобы рамка (24-33) рисовалась ВСЕГДА при обновлении экрана
_orig_flip = pygame.display.flip


def _flip_and_markers():
    draw_corner_markers()  # <-- Вот здесь рисуются маркеры 24-33
    _orig_flip()
...
# ────────── ФУНКЦИЯ ДЛЯ СКРИНШОТА (ОБНОВЛЕННАЯ) ──────────
def wait_for_screenshot_setup(marker_folder, scale=3.0):
    """
    Показывает AprilTags (0-8).
    Маркеры (24-33) добавятся автоматически при вызове pygame.display.flip().
    """
    print("\n" + "=" * 50)
    ...
    for d in degs:
        if d == 0:
            ...
            continue
        dx = deg2px(d)
        positions.append((marker_id, cx - dx, cy))  # Left
        marker_id += 1
        positions.append((marker_id, cx + dx, cy))  # Right
        marker_id += 1
    ...
    while waiting:
        ...
        # Flip автоматически нарисует 24-33 поверх этого
        pygame.display.flip()
...
# ─────────────────────────────── Main ────────────────────────────
async def main():
    global device, recording_running, time_offset_ns

    fname = f"EXP_{group_id}_{subject_id}_S{session}_{datetime.now():%Y%m%d_%H%M}.csv"
    logfile = open(fname, "w", newline="", encoding="utf-8")
    ...
    writer.writerow([])
    writer.writerow(["task_name", "dot_type", "dot_color", "dot_is_correct", "t_start", "t_end", "x_px", "y_px"])

    markers_path = "/Users/iliachapliev/Desktop/маркеры"

    # ❗ ШАГ 1: Показываем ВСЕ маркеры (0-8 + Рамка 24-33)
    wait_for_screenshot_setup(markers_path, scale=3.0)

    # ❗ ШАГ 2: Подключение по IP (с сохранением истории)
    CONFIG_FILE = "pupil_ip_config.json"
    last_ip = "192.168.1.101"  # Значение по умолчанию

    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r") as f:
                last_ip = json.load(f).get("ip", last_ip)
        except Exception:
            pass

    print("\n==========================================")
    print(f"🌐 Последний использованный IP: {last_ip}")
    user_ip = input("Введите новый IP телефона (или нажмите Enter, чтобы оставить текущий): ").strip()

    current_ip = user_ip if user_ip else last_ip

    # Сохраняем актуальный IP для следующих запусков
    ...
    try:
        # Прямая инициализация устройства через async API
        device = Device(address=current_ip, port=8080)

        # Делаем тестовый запрос статуса, чтобы убедиться, что связь есть
    ...
    async def send_event_and_log(tag: str, ts: int | None = None):
        ...
        writer.writerow(
            [task_name, core, dot_color, correct, ts if is_start else None, ts if is_end else None, None, None])
    ...
    blocks = [run_gap_task, run_overlap_task, run_prediction_task, run_decision_task, run_antisaccade_task]
    random.shuffle(blocks)
    for blk in blocks: await blk(writer, n_trials=20)
